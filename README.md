...
![四个核心技能与内容工作流](docs/workflow-overview.svg)

虚线表示人工衔接或按需调用，不代表四个技能会自动相互触发。选题决策、创作、制作与发布仍由人推进和确认。
...
```mermaid
flowchart TB
    X["X 公开链接"] --> XR["X 读取：原文、上下文、图视频"]
    XR -. "人判断并继续研究" .-> IDEA["选题、创作与制作"]
    E["周报、白名单消息、项目事实、日程"] --> PA["个人助理：累计状态与今日行动"]
    PA -. "人推进" .-> IDEA
    IDEA -. "人确认实际发布" .-> PUB["真实发布事件"]
    PUB -. "明确请求爬楼" .-> DY["抖音评论与全时长弹幕"]
    PUB -. "满足触发与授权条件" .-> MT["双平台 2h / 6h / 24h / 48h 快照"]
    DY --> D["联合报告与群卡片"]
    MT --> M["同一项目文档与完整复盘"]
    D -. "人确认有用反馈" .-> NEXT["下一轮选题与行动"]
    M -. "人确认有用结论" .-> NEXT
```

- 输入有来源：原帖、评论、工作证据与后台快照各自保留出处。
- 产物有落点：摘要、个人面板、飞书报告和项目文档承担不同用途。
- 完成有核对：回读、幂等键和校验帮助避免重复发送、覆盖历史和错误完成。
- 缺口如实保留：计划不算完成，读取失败不记为零，不虚构未读到的内容。
